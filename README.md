...
- **Per-account policy.** Three knobs let one account be spent differently from the rest. `cswap threshold <num|email> <pct>` gives that account its own switch-away line, used in place of the global one on both sides of a switch — the engine leaves an account when *its own* line is reached, and refuses to land on a candidate already past *its own*. `cswap standby <num|email>` holds an account back as a reserve: it is skipped while any other account can still be used, and taken only when none can — after which auto-switch returns to a primary as soon as one recovers. `cswap order <num|email> <rank>` pins that account's position in the switch chain: a lower rank is spent first, and every pinned account sorts ahead of every unpinned one. A pin is a leading tier on the ranking, not a tie-break — *within* a tier the active strategy still decides, so pins compose with `consume-first` rather than replacing it, and unpinned accounts go on being ranked by strategy alone. All three are per-account overrides rather than global settings, so an account you never touch keeps behaving exactly as it does today. `cswap threshold` and `cswap order` with no arguments print the global default with every override, and the resolved chain; the [TUI](#interactive-dashboard-tui) shows `th 85%`, `(standby)` and `ord 1` badges beside the account, and also lets you set all three in place (menu → *Per-account policy…*, then the account). Clear any of them with `cswap threshold <num|email> --unset`, `cswap order <num|email> --unset`, or `cswap unstandby <num|email>`.
  ...
  *by the active ranking strategy* because it carries no pin; being a reserve is what keeps it
  ...
  gains `"threshold": 85.0` and `"order": 1`, account 3's gains `"standby": true` — and
  ...
  Everything above is editable from the dashboard too: run `cswap`, choose *Per-account policy…*
  from the menu and pick the account. One form holds all three fields, prefilled with what that
  account carries right now; emptying a field clears the override exactly as `--unset` does, and
  a value the commands above would refuse is refused there with the same message.
...
Run `cswap` on its own (or `cswap tui`) for the full-screen dashboard: live usage for every account, switching, per-account policy editing, and the auto-switcher, all keyboard-driven. `cswap watch` opens it straight to the live monitor. Works on macOS, Linux, and Windows.
